# URF.NET &nbsp;&nbsp; [![NuGet Badge](https://buildstats.info/nuget/Urf.Repository.Pattern?includePreReleases=true)](https://www.nuget.org/packages?q=urf) #
...
### Roadmap: URF .NET Core (alpha-release) ETA: February 2018 ###
  ...
  * Quickly get up and running with URF under a minute, we've recently just started publishing [URF to NuGet](https://www.nuget.org/packages?q=urf), we're already approaching 1000 downloads. Shipping URF as NuGet packages is definitely something we did not realize as an added value. We were mistaken that most teams would like to clone/fork and include URF projects directly in their solution for infinite customizations possibilities however this was actually not the case. In reality, most teams use URF as is, right out the box, with zero customizations.
